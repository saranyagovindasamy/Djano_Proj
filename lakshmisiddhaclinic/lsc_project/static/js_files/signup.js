...
function suggest_username(){
    ...
    $.ajax({
		...
		success: function(response) {
            ...
            if (result == 409)
            {
                let su = $('#userprofile_tocreate').find(":selected").text()
                $('#modal-title').html('<i class="fas fa-2x text-danger fa-exclamation-circle"></i>' + '  ' + "<span style='font-size: 30px;'class='text-danger'> <b> Warning </b> </span>")
                $('#body-modal').html('User Login exist Already for ' + su + ' as ' +user_name);
